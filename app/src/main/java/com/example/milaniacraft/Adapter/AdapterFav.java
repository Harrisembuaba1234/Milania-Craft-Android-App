...
import com.example.milaniacraft.DetailFav;
...
public class AdapterFav extends RecyclerView.Adapter<AdapterFav.HolderDataCari> {
    Context ctx;
    List<DataTampilFav> listCariData;
    Intent intent;

    public AdapterFav(Context ctx, List<DataTampilFav> listCariData, Intent intent) {
        this.ctx = ctx;
    ...
    @NonNull
    @Override
    public AdapterFav.HolderDataCari onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View layout = LayoutInflater.from(ctx).inflate(R.layout.card_fav, parent, false);
        return new AdapterFav.HolderDataCari(layout);
    }

    @Override
    public void onBindViewHolder(@NonNull AdapterFav.HolderDataCari holder, @SuppressLint("RecyclerView") int position) {
        DataTampilFav db = listCariData.get(position);
        ...
        Picasso.get().load(ApiClient.IMAGES_URL+listCariData.get(position).getImage()).resize(150,157).error(R.mipmap.ic_launcher).into(holder.ivIcon);
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Intent mIntent =  new Intent(view.getContext(), DetailFav.class);
    ...
    public class HolderDataCari extends RecyclerView.ViewHolder {
        TextView tvId, tvJenis,tvNama,tvUkuran, tvHarga;
        ImageView ivIcon;

        public HolderDataCari(@NonNull View itemView) {
            super(itemView);

            tvJenis = itemView.findViewById(R.id.namaBarang);
            tvNama = itemView.findViewById(R.id.NamaJenis);
            ivIcon = itemView.findViewById(R.id.NamaFoto);
            tvHarga= itemView.findViewById(R.id.NamaHarga);
        }
    }
